"""
Affinetes-compatible ManiSkill3 evaluation environment.

This Actor class runs inside an affinetes-managed container and:
1. Manages SAPIEN/ManiSkill3 robotics simulation
2. Queries miner policy endpoints (on Basilica or self-hosted) for actions
3. Returns evaluation scores

Supported environments:
- maniskill3/pick-cube-v1: Pick up a cube
- maniskill3/stack-cube-v1: Stack one cube on another
- maniskill3/peg-insertion-v1: Insert peg into hole
- maniskill3/push-cube-v1: Push cube to target
- maniskill3/pull-cube-v1: Pull cube to target
- maniskill3/lift-cube-v1: Lift cube to target height
- maniskill3/turn-faucet-v1: Turn a faucet handle
- maniskill3/open-cabinet-door-v1: Open a cabinet door

Usage (from backend):
    import affinetes as af_env

    env = af_env.load_env(image="kinitro/maniskill3:v1")

    result = await env.evaluate(
        task_id=123,
        base_url="https://xxx.deployments.basilica.ai",
        env_id="maniskill3/pick-cube-v1"
    )
"""
...
class Actor:
    """
    ManiSkill3 evaluation actor for affinetes.

    Runs SAPIEN/ManiSkill3 simulation and queries miner policy endpoints
    ...
    async def list_environments(self) -> list[str]:
        """List available ManiSkill3 environments."""
        return [env_id for env_id in get_all_environment_ids() if env_id.startswith("maniskill3/")]
    ...
    async def evaluate(
        ...
        env_id: str = "maniskill3/pick-cube-v1",
        ...
        **kwargs,
        ...
        """
        Evaluate a miner's policy on a ManiSkill3 task.
        ...
        Args:
            ...
            env_id: Environment ID (e.g., "maniskill3/pick-cube-v1")
        ...
        """
        # Validate env_id is a maniskill3 environment
        if not env_id.startswith("maniskill3/"):
            return self._build_error_result(
                env_id=env_id,
                task_id=task_id,
                seed=seed or task_id,
                start_time=time.time(),
                error=f"Invalid env_id for ManiSkill3 container: {env_id}. "
                "Must start with 'maniskill3/'",
            )
