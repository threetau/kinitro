...
class AgentLoader:
    """
    Loads miner-submitted agent implementations safely.
    ...
    def _install_requirements(submission_dir: Path) -> None:
        ...
        if cache_file.exists() and not is_ray_worker:
            try:
                ...
                if time.time() - last_install < 3600:  # 1 hour
                    print(f"📦 Requirements recently installed, skipping (cache: {cache_file})", flush=True)
                    return
            except Exception:
                pass
        elif is_ray_worker:
            print("📦 Ray worker detected, forcing requirements installation", flush=True)
        ...
        lerobot_req: Optional[str] = next((ln for ln in requirement_lines if ln.startswith("lerobot")), None)
        ...
        try:
            ...
            try:
                ...
                if lerobot_req is not None:
                    if is_ray_worker:
                        print(f"Ray worker: Installing with all deps: {lerobot_req}", flush=True)
                    ...
                    if return_code != 0:
                        raise subprocess.CalledProcessError(return_code, "uv pip install --no-deps lerobot")

                # Next, install the remaining requirements normally
                if other_reqs:
                    tmp_req_path = None
                    try:
                        with tempfile.NamedTemporaryFile(mode="w", delete=False) as tmp_req:
                            tmp_req.write("\n".join(other_reqs) + "\n")
                            tmp_req_path = tmp_req.name

                        process = subprocess.Popen(
                            ["uv", "pip", "install", "--force-reinstall", "-r", tmp_req_path],
                            stdout=subprocess.PIPE,
                            stderr=subprocess.STDOUT,
                            text=True,
                            bufsize=1,
                        ...
                        return_code = process.poll()
                        if return_code != 0:
                            raise subprocess.CalledProcessError(return_code, "uv pip install -r <temp>")
                    finally:
                ...
                if isinstance(e, FileNotFoundError):
                    ...
                    print(f"⚠️  uv installation failed (exit code {e.returncode}), falling back to pip...", flush=True)
                ...
                if lerobot_req is not None:
                    process = subprocess.Popen(
                        [python_executable, "-m", "pip", "install", "--no-deps", lerobot_req],
                        stdout=subprocess.PIPE,
                        stderr=subprocess.STDOUT,
                        text=True,
                        bufsize=1,
                    ...
                    return_code = process.poll()
                    if return_code != 0:
                        raise subprocess.CalledProcessError(return_code, "pip install --no-deps lerobot")
                ...
                if other_reqs:
                    ...
                    try:
                        ...
                        process = subprocess.Popen(
                            [python_executable, "-m", "pip", "install", "-r", tmp_req_path],
                            stdout=subprocess.PIPE,
                            stderr=subprocess.STDOUT,
                            text=True,
                            bufsize=1,
                        ...
                        return_code = process.poll()
                        if return_code != 0:
                            raise subprocess.CalledProcessError(return_code, "pip install -r <temp>")
                    finally:
                        if tmp_req_path and os.path.exists(tmp_req_path):
                            try:
                                os.unlink(tmp_req_path)
            ...
            print(f"❌ Failed to install requirements (exit code {e.returncode})", flush=True)
            print("Proceeding anyway - agent may fail if dependencies are missing", flush=True)
        except Exception as e:
    ...
    def prefetch_models_if_configured(submission_dir: Path) -> None:
        ...
        try:
            ...
            if path_str.startswith(".") or path_str.startswith("/") or os.sep in path_str:
                return
            ...
            try:
                ...
                print(f"⚠️  huggingface_hub not available yet ({imp_exc}); skipping prefetch", flush=True)
                return
