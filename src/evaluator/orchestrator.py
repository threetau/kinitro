"""
Orchestrator - Executor-based task orchestration.

Uses the TaskExecutor pattern for pluggable task type support. Maintains
backward compatibility with the existing EvalJobMessage format while adding
support for the new TaskSpec-based approach.

Connects directly to the backend via WebSocket to receive jobs and send results.
"""
...
class Orchestrator:
    ...
    Uses the TaskExecutor pattern to support multiple task types. Delegates
    setup, execution, and teardown to registered executors, allowing new task
    types to be added without modifying the orchestrator itself.
    ...
    def __init__(self, config: EvaluatorConfig):
        self.config = config
        logger.info("Orchestrator initialized with db: %s", self.config.pg_database)
        ...
        logger.info("Orchestrator initialized with config: %s", self.config)
    ...
    async def start(self) -> None:
        ...
        logger.info(
            "Orchestrator connecting to backend (max concurrent: %s)...",
            self.max_concurrent_jobs,
        )
